...
,"s":[["kyarr","က်္ား","$","",-1],["kyunote","ကြၽႏု္ပ္","$","",-1],["nhite","၌","","",-1],["hnite","၌","","",-1],["shat","ယွက္","$","",-1],["nyin","ညာဥ္","$","",-1],["shin","ယွဥ္","$","",-1],["kyar","က်္ာ","$","",-1],["yway","၍","","",-1],["umm","အမ္","",""],["imm","အင္းမ္...","",""],["yin","ယာဥ္","","",-1],["yin","ယ်ာဥ္","","",-1],["ywe","၍","","",-1],["d","ဒီ","",""],["u","ယူ","",""],["own","အံုး","$","",-1],["it","ဧတ္","",""],["el","ဧည့္","",""],["ei","ဣ","",""],["or","ဪ","$","",-1],["ei","၏","",""],["ei","ဤ","",""],["oo","ဥ","",""],["ah","အ","",""],["aw","ဪ","",""],["ay","ဧ","",""],["ag","ေအာင္","",""],["oo","ဦး","",""],["oh","အိုး","",""],["r","အာ","",""],["ae","အဲ","",""],["ei","အိ","",""],["ei","အီ","",""],["um","အမ္","",""],[".","။","",""],[",","၊","",""],[",","ျပီး","",""],[".","ျပီ","",""],["4","၎","-1","",-1],[".","ဤ","",""],[".","သည္","",""],[".","၏","",""],[",","၌","",""],[",","၍","",""],[",","ႏွင့္","$",""],["f","္","","--္",-1],["b","ျပီ","",""],["o","အို","",""],["p","ျပီ","",""],["e","ဤ","",""],["a","အ","",""],["a","ေအ","","",-1],["u","ဥ","",""],["u","ဦး","",""],["h","့","","--့",-1],[";","း","",""],["eu","အူ","",""],["u","အူ","",""],["u","အု","",""],["a","အစ္","","",-1],["tun","ထြန္း","",""]]
,"f":["[ဍဋဌ][ႇွုူႈႉြႊ]","[ဠ][ြႊ]","[ၠၡၢၣၥၧၨၩၬၭၰၲၱၳၴၵၶၷၸၹၺၻၼႅ႒][ႇွုူႈႉြႊ]"]
};
...
function _getMenuExUpd(obj, query, data){
	...
	//convert font in burglish menu
	if(_o[_o.x]["font"]!="Zawgyi_One"){
		for(var i2=0;i2<data.length;i2++){
			data[i2][1]=data[i2][1].convertFont("Zawgyi_One",_o[_o.x]["font"]);
			data[i2][3]=data[i2][3].convertFont("Zawgyi_One",_o[_o.x]["font"]);
		}
	}
...
function _getTip(){
	var ret=_tips[Math.round(Math.random()*(_tips.length-1))];
	if(_o[_o.x]["font"]!="Zawgyi_One"){
		ret=ret.convertFont("Zawgyi_One",_o[_o.x]["font"]);
	}
    return ret;
}
...
{
   e.originalTarget.className = "burglish_off";
}

//Burglish Menu Item Generator
function _showMenu( obj, query, data){
	var itemCount = 0;
	var dropdown = _id( mmgeeks.currentId + "drop" );
	
	html = mmgeeks.doc.createElement("DIV");
	if (mmgeeks.showTips)
	{
	   tip = mmgeeks.doc.createElement("DIV");
	   tip.setAttribute("id","help"+ mmgeeks.currentId);
	   tip.setAttribute("style",style_tips.replace("_FONT_",str(_f[_dfont]["css"])));
	   tip.innerHTML = _getTip();
	   html.appendChild(tip);
	}
   ...
   for(var i = 0; i < data.length ; i++){
      if (i % 10 == 0)      
      {
         if (currentCol != null)
         {
            html.appendChild(currentCol);
         }
         col = mmgeeks.doc.createElement("DIV");
         col.setAttribute("id","col"+ mmgeeks.currentId + (i/10));
         if (i == 0)
            col.setAttribute("style",style_colon);
         else 
            col.setAttribute("style",style_coloff);
         
         currentCol = col;
      }
      
      if( mmgeeks.burmese ) {
         var item = mmgeeks.doc.createElement("DIV");
         item.setAttribute("id","item"+ mmgeeks.currentId+i);
	      item.innerHTML = '&nbsp;' + (i%10) + '. ' + data[i][0] + ' ' + data[i][1];
	      item.addEventListener("mousedown",_onMDown_,false);
	      item.addEventListener("mouseup",_onMUp_,false);
	      item.addEventListener("mouseover",mouseOverItem,false);
         item.addEventListener("mouseout",itemMouseOut,false);
         currentCol.appendChild(item);
		}else if( mmgeeks.burglish ){
			if(obj.conv) data[i][1] = data[i][1].convertFont(obj.conv[0],obj.conv[1]);
			
         var item = mmgeeks.doc.createElement("DIV");
         item.setAttribute("id","item"+ mmgeeks.currentId+i);
	      item.innerHTML = '&nbsp;' + (i%10) + '. ' + data[i][0].replace( query , '<b>' + query + '</b>' ) + ' ' + (data[i][3]?data[i][3]:data[i][1]);
	      item.addEventListener("mousedown",_onMDown_,false);
	      item.addEventListener("mouseup",_onMUp_,false);
	      item.addEventListener("mouseover",mouseOverItem,false);
         item.addEventListener("mouseout",itemMouseOut,false);
         ...
         var item = mmgeeks.doc.createElement("DIV");
         item.setAttribute("id","item"+ mmgeeks.currentId+i);
	      item.innerHTML = '&nbsp;' + (i%10) + '. ' + data[i].replace( query , '<b>' + query + '</b>' );
	      item.addEventListener("mousedown",_onMDown_,false);
	      item.addEventListener("mouseup",_onMUp_,false);
	      item.addEventListener("mouseover",mouseOverItem,false);
         item.addEventListener("mouseout",itemMouseOut,false);
   ...
   if (data.length == 0) 
      dropdown.innerHTML = "&lt;No Items available>";
   else
   {
      html.appendChild(currentCol);
      _show( mmgeeks.currentId + "drop");
		_o[mmgeeks.currentId]["curPos"] = -1;
		dropdown.innerHTML = "";
	   dropdown.appendChild(html);
   }
   _o[mmgeeks.currentId]["count"] = data.length;
	...
	if(!isIE){
		...
		var _top2=0;
		if(mmgeeks.doc.body.clientHeight+mmgeeks.doc.body.scrollTop > _id(_o.x).offsetTop + _id(_o.x).offsetHeight + dropdown.offsetHeight ){
...
function _onKDown_(e){   
   ...
   this.currentId = obj.id;
	...
	if( key==13 ) {
		if( _o.ctrl || (!mmgeeks.burglish && !_o[_o.x]["_suggest"] && !_o[_o.x]["_binput"]) ) { 
		...
		//_o[_o.x]["_bflag"]++;
		//_o[_o.x]["_bmenu"]=(_o[_o.x]["_bflag"]%3==1);
		//_o[_o.x]["_binput"]=(_o[_o.x]["_bflag"]%3==2);
		return false;
	}else if (key == 81 && _o.alt)
	{
   	sh_chlist();
	   return false;
	}
...
function _onMDown_(){
    var obj=_id(mmgeeks.currentId);
    var curPos=_o[_o.x]["curPos"];//==-1?0:_o[_o.x]["curPos"];
    if(curPos==-1 && mmgeeks.burglish) curPos=0; //if burglish menu and not selecting anything, just choose it first one.
...
function setFocus( index ){
	...
	for(var i = 0; i < _o[_o.x]["count"]; i++ ){
	 ...
	 for(var i =0; i < Math.ceil( _o[_o.x]["count"] / 10); i++){
        _id("col"+_o.x+i).className = i==Math.floor(_o[_o.x]["curPos"]/10)?"burglish_colon":"burglish_coloff";
    }
	_show(_o.x + "drop");
...
function mouseOverItem(e)
{
   ...
   var prefix = "item"+mmgeeks.currentId;
   var index = id.substr(prefix.length);
   setFocus( index );
}
...
function onFocus( e ){
	mmgeeks.currentId = e.originalTarget.id;
}

function storePosition(_g) {
	if( mmgeeks.doc.selection ){ 
		var range = mmgeeks.doc.selection.createRange();
...
function writeTextbox( _g ){
	...
	mmgeeks.doc.body.innerHTML += '<span id="'+_g.id+'drop"'+' class="burglish_inline">'+'</span>';
}
...
function attachTextbox( _g ){
	...
	obj.setAttribute("burglish","true");	

   obj.addEventListener("keydown",_onKDown_,false);
   obj.addEventListener("keyup",_onKUp_,false);
   obj.addEventListener("keypress",_onKPress_,false);   
//   obj.addEventListener("mouseover",_onMOver_,false);
   obj.addEventListener("blur",onBlur,true);
   obj.addEventListener("focus",onFocus,true);

   var span = mmgeeks.doc.createElement('div');
   span.setAttribute("style",style_inline.replace("_FONT_",str(_f[_dfont]["css"])));
   span.id = _g.id + "drop";
   _parNode.insertBefore(span, obj.nextSibling);   
...
//var tmpkeypress = mmgeeks.doc.onkeypress?mmgeeks.doc.onkeypress:function(){};
//if not IE, hook all keypress
//if(!isIE) mmgeeks.doc.onkeypress = _onKPress_;
...
function chgfont(_x){
	...
	_id("help"+_o.x).style.fontFamily =_id(_o.x+"drop").style.fontFamily=str(_f[_x]["css"]);
	_id("help"+_o.x).style.fontSize =_id(_o.x+"drop").style.fontSize = _f[_x].fontsize+"pt";
...
//insert character
function _ic(_s){
   if (!mmgeeks.currentId) return;
	var _g = _id(mmgeeks.currentId);
	_Wrap(_g,_s);
	_g.focus();
}
...
function initTextarea( _g, skipAttachment ) {
	...
	if (!skipAttachment) {
      if(_id(_g.id).tagName=="TEXTAREA" || _id(_g.id).tagName=="INPUT" )	attachTextbox( _g ); 	
	}	
	
	_dfont=_o[_g.id]["font"];
};

function _characterClick(e)
{
   var obj = e.originalTarget;
   var ch = obj.innerHTML.replace(/[\-\s]+/g,'');
   _ic(ch);
}
...
function initCharacterList()
{
	...
	div = mmgeeks.doc.createElement('div');
	div.setAttribute("class","burglish_chlist dragme");
	div.setAttribute("id","bchlist");
	div.setAttribute("style","width:90%;display:none;left:5%;position:fixed;bottom:5px");
		
	for(var i=0;i<_chlist.length;i++){
		var _curch=_chlist[i].replace(/[\-\s]+/g,'');
		var span = mmgeeks.doc.createElement("span");
		span.addEventListener("click",_characterClick,false);
		span.innerHTML = _chlist[i];
		div.appendChild(span);
	}

	mmgeeks.doc.body.appendChild(div);
}
