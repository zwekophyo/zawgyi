...
class Fetcher(webapp.RequestHandler):
  ...
  def get(self):
    ...
    self.response.out.write("""<!DOCTYPE html><html><head>
         ...
         <link rel="stylesheet" type="text/css" 
               href="/stylesheets/feedfetcher.css"/>
         </head><body>""")
  ...
  def GenerateScopeRequestLink(self, scope):
    return self.client.GenerateAuthSubURL(
        'http://' + HOST_NAME + '/?token_scope=%s' % (scope),
  ...
  def DisplayAuthorizedUrls(self):
    ...
    stored_tokens = StoredToken.gql('WHERE user_email = :1', 
        self.current_user.email())
    for token in stored_tokens:
        self.response.out.write('<li><a href="%s/?feed_url=%s">%s*</a></li>' % (
            URL_TAG, urllib.quote_plus(token.target_url), token.target_url))
    self.response.out.write(
...
def main():
  application = webapp.WSGIApplication([('/newfeed.html', Fetcher), ('/google72db3d6838b4c438.html', Acker)], debug=True)
